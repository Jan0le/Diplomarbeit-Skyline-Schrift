...
Im Folgenden werden Ziele, typische Datenquellen und technische Anforderungen des automatisierten Flugdatenimports erläutert.

\subsection{Zielsetzung und Nutzen}

Der automatisierte Import von Flugdaten verfolgt das übergeordnete Ziel, repetitive manuelle Dateneingaben zu eliminieren und die Qualität der erfassten Informationen signifikant zu verbessern.
Anstatt jeden Flug manuell einzupflegen, erkennt das System relevante Daten automatisch aus verschiedenen Quellen -- sei es ein QR-Code auf dem Boarding Pass, ein Foto eines Papierdokuments oder eine E-Mail-Buchungsbestätigung.
Dies entlastet den Nutzer erheblich, da insbesondere vielreisende Personen andernfalls dutzende Felder pro Flug manuell befüllen müssten.
Gleichzeitig sinkt die Fehlerquote, da Tipp- und Übertragungsfehler, die bei manueller Eingabe unvermeidlich auftreten, durch den automatisierten Prozess weitgehend vermieden werden.

\subsection{Abgrenzung zur manuellen Erfassung}

Die manuelle Erfassung bleibt als Fallback-Option unverzichtbar und muss parallel zum automatisierten Import unterstützt werden -- etwa für Flüge, bei denen kein maschinenlesbares Dokument vorliegt, oder wenn der Nutzer Korrekturen vornehmen möchte.
Allerdings ist sie zeitaufwändiger und mit einem höheren Fehlerrisiko verbunden.
Der automatisierte Weg steigert Komfort, Konsistenz und Wiederholbarkeit: Ein einmal konfiguriertes Importschema funktioniert für hunderte Flüge gleichermaßen zuverlässig.

\subsection{Typische Fehlerquellen bei Importen}

Beim automatisierten Import treten verschiedene Fehlerklassen auf.
Unvollständige Daten entstehen, wenn Quelldokumente nicht alle erforderlichen Felder enthalten -- beispielsweise fehlen bei manchen mobilen Boarding Passes Gate-Nummern oder Sitzplatzdaten.
Fehlerhafte OCR-Erkennung ist eine weitere häufige Ursache: Bildqualität, Kontrast, Schriftgröße und Schiefstellung beeinflussen, ob Zeichen korrekt erkannt werden.
Und schließlich können QR-Codes oder BCBP-Datenfelder nicht standardkonform kodiert sein, was beim Parsing zu Fehlzuordnungen führt.
Daher sind robuste Validierungsschritte nach dem Einlesen sowie die Möglichkeit zur manuellen Nachkorrektur unbedingt vorzusehen.

\subsection{Datenquellen für Flugdaten}

\subsubsection{QR-Code / Boarding Pass (BCBP)}

Der Bar Coded Boarding Pass (BCBP) ist ein von der IATA (International Air Transport Association) definierter Standard zur strukturierten Codierung von Passagier- und Flugdaten in einem zweidimensionalen Barcode.
Die zugrunde liegende Spezifikation ist in der IATA Resolution~792 festgelegt und wurde seit ihrer Einführung im Jahr~2005 mehrfach aktualisiert -- zuletzt in Version~8, die seit dem 1.~Juni 2020 gültig ist \cite{iataCommonUse}.

Das Format basiert auf fixen Positionsfeldern innerhalb eines Datenstrings.
Der Standard definiert PDF417 als Barcode-Symbologie für gedruckte Boarding Passes sowie Aztec, Datamatrix und QR~Code für mobile Boarding Passes \cite{iata2021_bcbp_implementation_guide}.
Ein einzelner Barcode kann dabei bis zu vier Flugsegmente gleichzeitig kodieren, was besonders bei Umsteigeverbindungen vorteilhaft ist.

Die Pflichtfelder umfassen den Abflug- und Zielflughafen (3-stellige IATA-Codes), die Flugnummer, das Flugdatum und den Passagiernamen.
Diese Kernfelder reichen aus, um einen validen Flug-Datensatz anzulegen.
Optionale Felder -- wie Sitzplatz, Gate, Buchungsreferenz, Frequent-Flyer-Nummer oder Sicherheitsinformationen -- sind zwar im Standard vorgesehen, werden aber nicht von allen Airlines befüllt \cite{ewingBoardingPassBarcode}.

% BCBP-Struktur: Beispielbild BCBP-Code (Boarding Pass). Quelle: Bubble Forum.
\IfFileExists{img/fehlende_bilder/bcbp_structure/image.png}{%
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{fehlende_bilder/bcbp_structure/image}
  \caption{BCBP Feldstruktur – Beispielbild eines BCBP-Codes (Boarding Pass). \cite{bubbleForumBCBP}}
  \label{fig:bcbp_structure}
\end{figure}%
}{}

\subsubsection{OCR aus Bildern/Dokumenten}

Optical Character Recognition (OCR) ermöglicht die Textextraktion aus Fotos oder eingescannten PDFs, wenn kein maschinenlesbarer QR-Code vorhanden ist.
OCR analysiert die Pixel eines Bildes, segmentiert einzelne Zeichen und gleicht diese mit bekannten Zeichenmustern ab -- moderne Systeme nutzen dafür neuronale Netze, die auf Millionen Zeichenbeispielen trainiert wurden \cite{parseaIoOcr}.

Die erreichbare Genauigkeit hängt stark von der Qualität der Vorlage ab: Saubere, hochauflösende digitale Dokumente erzielen Erkennungsraten von 98--99\,\%, während schlechte Scans oder beschädigte Dokumente nur auf 75--85\,\% kommen \cite{parseaIoOcr}.
Konkret beeinflussen folgende Faktoren die OCR-Qualität \cite{docsumoOcrAccuracy}:
\begin{itemize}
  \item \textbf{Auflösung:} Mindestens 300~DPI werden für zuverlässige Erkennung empfohlen; bei kleinen Schriftgrößen sind 400--600~DPI vorzuziehen \cite{pittOcrBestPractices}.
  \item \textbf{Kontrast:} Schwarze Schrift auf weißem Hintergrund liefert die besten Ergebnisse; farbige oder blasse Tinten senken die Treffsicherheit erheblich.
  \item \textbf{Schiefstellung (Skew):} Gedrehte oder verzerrte Dokumente führen zu Segmentierungsfehlern; Vorab-Begradigung (Deskewing) verbessert die Ergebnisse.
  \item \textbf{Bildrauschen und Artefakte:} Flecken, Falten oder Kompressionsartefakte können Zeichen verfälschen.
\end{itemize}

Nach der OCR müssen die extrahierten Texte geparst und relevanten Feldern zugeordnet werden.
Plausibilitätsprüfungen -- etwa die Überprüfung ob ein erkannter Code einem validen IATA-Flughafencode entspricht oder ob ein Datum im korrekten Format vorliegt -- sind dabei essenziell, um fehlerhafte OCR-Ausgaben frühzeitig abzufangen.

% OCR-Pipeline: \url{https://theailearner.com} (OCR-pipeline.png) herunterladen und als ocr_pipeline.png in content/img/ ablegen.
\IfFileExists{img/fehlende_bilder/ocr_pipeline/IMG_5191.PNG}{%
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{IMG_5191}
  \caption{OCR-Prozess (Pipeline). \cite{theailearnerOCR}}
...
Buchungsbestätigungs-E-Mails von Airlines und Reisebuchungsportalen enthalten strukturierte Informationen wie Buchungsreferenz (PNR), Routing (Abflug- und Zielflughafen), geplante Abflug- und Ankunftszeiten sowie Passagierdaten.
Diese Informationen lassen sich mittels Parser automatisiert extrahieren.

Die wesentliche Herausforderung besteht in der Heterogenität der Vorlagen: Jede Airline und jedes Buchungsportal verwendet ein eigenes E-Mail-Template mit unterschiedlichem Layout, Sprachstil und Zeichencodierung.
Dies erfordert entweder eine umfangreiche Sammlung airline-spezifischer Parsing-Regeln oder den Einsatz heuristischer Verfahren, die häufige Muster (z.\,B.\ \glqq Flight: LH~441\grqq{}, \glqq Departure: FRA\grqq{}) erkennen können.
Alternativ bieten neuere Ansätze den Einsatz von KI-Modellen, die natürlichsprachige Buchungsbestätigungen kontextbasiert verstehen können.

\subsection{Technische Anforderungen}

Damit das Import-System im Produktivbetrieb zuverlässig funktioniert, müssen mehrere technische Anforderungen erfüllt sein:

\textbf{Validierung:} Alle importierten Daten müssen auf Plausibilität geprüft werden.
Dies umfasst mindestens die Überprüfung von IATA-Flughafencodes auf Existenz in einer bekannten Airport-Datenbank, die Prüfung von Datumsformaten und die Sicherstellung, dass Abflugdatum chronologisch vor dem Ankunftsdatum liegt.

\textbf{Feld-Mapping und Normalisierung:} Externe Datenquellen liefern Felder in unterschiedlichen Formaten (z.\,B.\ verschiedene Datumsschreibweisen, mehrteilige Passagiernamen).
Ein einheitliches Mapping normalisiert diese auf das interne Datenmodell.

\textbf{Deduplizierung:} Das System muss erkennen, wenn ein Flug bereits in der Datenbank existiert, um Mehrfachimporte zu verhindern.
Kriterien hierfür können Kombination aus Flugnummer, Datum und Route sein.

\textbf{Fallback-Strategien:} Können nicht alle Pflichtfelder automatisch befüllt werden, muss der Nutzer die Möglichkeit erhalten, fehlende Informationen manuell zu ergänzen, bevor der Import abgeschlossen wird.

\textbf{Asynchrone Verarbeitung:} Insbesondere bei OCR-lastigen Operationen kann die Verarbeitungszeit mehrere Sekunden betragen.
Die UI darf während dieser Zeit nicht einfrieren; die Verarbeitung ist daher asynchron zu gestalten, mit entsprechendem Ladefeedback für den Nutzer.

\textbf{Nutzerfreundliche Fehlerbehandlung:} Auftretende Fehler -- etwa ein nicht erkannter QR-Code oder ein unvollständiges OCR-Ergebnis -- müssen dem Nutzer verständlich kommuniziert werden, mit klaren Hinweisen, welche Felder manuell korrigiert werden müssen.

\subsection{Vergleich: Manuell vs.\ Automatisch}

\begin{table}[H]
  \centering
  \caption{Vergleich manueller und automatisierter Flugdaten-Import}
  \label{tab:import_vergleich}
  \begin{tabular}{p{4.5cm}p{4.2cm}p{4.2cm}}
    \toprule
    \textbf{Kriterium} & \textbf{Manuell} & \textbf{Automatisch} \\
    \midrule
    Zeitaufwand & Hoch (mehrere Minuten je Flug) & Gering (Sekunden) \\
    Fehlerquote & Höher (Tippfehler) & Niedriger (mit Validierung) \\
...
Automatisierter Import ist insbesondere für vielreisende Nutzer mit hohem Datenvolumen deutlich überlegen.
Manueller Import bleibt jedoch als unverzichtbarer Fallback und für Korrekturen bestehen.

\section{Grundlagen visueller Flugrouten-Darstellung}

Die visuelle Darstellung von Flugrouten auf einer Karte ergänzt tabellarische Fluglisten um eine räumliche Perspektive; die folgenden Abschnitte fassen Nutzen, Anforderungen und Berechnungsgrundlagen zusammen.

\subsection{Charakteristika und Nutzen}

Die visuelle Darstellung von Flugrouten auf einer interaktiven Karte dient der räumlichen Orientierung und bietet dem Nutzer eine intuitive Übersicht über seine Reisehistorie und geplante Flüge.
Anstatt Flüge als abstrakte Liste zu betrachten, können Nutzer ihre zurückgelegten Wege auf einem Globus nachvollziehen, Entfernungen einschätzen und regionale Schwerpunkte ihrer Reisetätigkeit erkennen.
Die Karte ergänzt damit die tabellarische Datensicht um eine wertvolle visuelle Dimension.

\subsection{Anforderungen an eine mobile Flugrouten-Karte}

Der Einsatz auf mobilen Endgeräten stellt besondere Anforderungen an das Kartensystem:

\textbf{Performance und Ladezeiten:} Die Karte muss auch bei vielen gespeicherten Flügen flüssig interagierbar sein.
Bibliotheken wie React Native Maps (auf iOS MapKit, auf Android Google Maps) nutzen GPU-beschleunigtes Rendering, das selbst bei hunderten Routen performant bleibt \cite{reactNativeMaps,mapboxMaps}.

\textbf{Mobile Optimierung:} Touch-Gesten (Pinch-to-Zoom, Drag) müssen natürlich reagieren.
Interaktive Elemente wie Airport-Marker müssen ausreichend groß für Fingerbedienung sein.

\textbf{Geodätische Korrektheit:} Flugrouten dürfen nicht als gerade Linien auf einer 2D-Karte eingezeichnet werden, da dies die tatsächliche Route verfälscht.
Stattdessen sind Großkreisbögen (Great Circle Arcs) zu verwenden, die die kürzeste Verbindung auf der Erdoberfläche geometrisch korrekt darstellen \cite{leafletArc}.

\textbf{Usability:} Zoom-to-Fit auf eine ausgewählte Route, Markierungen für Start- und Zielflughafen sowie Tooltip-Informationen beim Antippen einer Route steigern die Bedienbarkeit erheblich.

\subsection{Technische Grundlagen der Routenberechnung}

\textbf{Koordinatenbasis:} Jedem Flughafen ist ein Koordinatenpaar (Längengrad $\lambda$, Breitengrad $\varphi$) zugeordnet.
Die Flugroute wird anhand dieser geografischen Koordinaten von Abflug- und Zielflughafen berechnet.

\textbf{Haversine-Formel:} Zur Bestimmung der Entfernung zwischen zwei Punkten auf der Erdoberfläche wird die Haversine-Formel verwendet \cite{wikipediaHaversine,movableTypeLatLong}:
\begin{equation}
...
\end{equation}
Dabei ist $R$ der mittlere Erdradius ($\approx 6{,}371$~km).
Die Haversine-Formel gilt als numerisch stabil auch bei sehr kleinen Distanzen und liefert für die kugelförmig angenäherte Erde eine Genauigkeit von besser als 0{,}5\,\% \cite{wikipediaGreatCircle}.
Für Anwendungsfälle, bei denen höhere Genauigkeit gefordert ist, kann stattdessen die Vincenty-Formel auf Basis eines Erdellipsoids verwendet werden.

\textbf{Great-Circle-Bogen:} Die Flugroute wird nicht als direkte Verbindungslinie auf der 2D-Karte dargestellt, sondern als Großkreisbogen (Great Circle Arc) modelliert.
Ein Großkreis ist der größte mögliche Kreis auf einer Kugeloberfläche und beschreibt die kürzeste Verbindung zwischen zwei Punkten -- analog einer gespannten Schnur auf einem Globus \cite{gisGeographyGreatCircle}.
In der Praxis weichen reale Flugrouten durch Luftraumrestriktionen, Wetterbedingungen und ETOPS-Anforderungen vom theoretischen Großkreis ab, dieser dient aber als gute Approximation für die visuelle Darstellung.

\textbf{Flugfortschritt in Echtzeit:} Für laufende Flüge kann der aktuelle Fortschritt über lineare Interpolation zwischen \texttt{departureAt} und \texttt{arrivalAt} ermittelt werden.
Der Fortschritt $p$ zum Zeitpunkt $t$ ergibt sich als:
\begin{equation}
  p = \frac{t - t_{\mathrm{dep}}}{t_{\mathrm{arr}} - t_{\mathrm{dep}}} \quad \in [0, 1]
\end{equation}
Dieser Wert kann dazu genutzt werden, einen animierten Punkt entlang des Großkreisbogens zu positionieren \cite{mapboxAnimatePoint}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{04_haversine_sketch/haversine-sketch}
  \caption{Haversine-Formel visuell. \cite{sketchplanationsHaversine}}
  \label{fig:haversine_sketch}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{"00_verbesserungen/push trigger notifications"}
  \caption{Beispiel einer geplanten Push-Benachrichtigung. \cite{cssTricksScheduledPush}}
...
\begin{figure}[H]
  ...
  \caption{Publish/Subscribe-Schema (Push-Prinzip): Der Server liefert Informationen proaktiv an den Client. \cite{wikimediaPushSubscribe}}
...
\begin{figure}[H]
  ...
  \caption{Polling-System (Pull-Prinzip): Der Client fragt periodisch beim Server ab. \cite{wikimediaPolling}}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{00_verbesserungen/notification-interaktion}
  \caption{Beispielhafte Notification-Interaktion in einer mobilen App (Einstellungen, Benachrichtigungen, Sprache). \cite{dribbbleNotificationInteraktion}}
...
\begin{figure}[H]
  ...
  \caption{Abflugtafel als Beispiel für Echtzeit-Fluginformationen (Verspätungen, Gate). \cite{wikimediaDepartureBoard}}
...
Dieser Abschnitt ordnet zentrale Datenhaltung, Konsistenz und Sicherheit im Kontext einer mobilen Reise-App ein und leitet konkrete Anforderungen ab.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[fill=white, inner sep=2pt, draw=none] at (3.4,1.1) {\scriptsize Login + PKCE};
    \node[fill=white, inner sep=2pt, draw=none] at (3.4,-0.05) {\scriptsize Access Token};
  \end{tikzpicture}
...
Aus den bisherigen Grundlagen lassen sich übergreifende Anforderungen an Datenfluss, Import, Validierung und Integration formulieren.
...
Abschließend werden die gewählten Technologien für Frontend, Backend und Architekturentscheidungen skizziert.
...
iOS und Android \cite{reactNative}. Skyline wurde dabei primär für \textbf{iOS} entwickelt und im \textbf{Apple App Store} verfügbar; Android wird als sekundäre Zielplattform
