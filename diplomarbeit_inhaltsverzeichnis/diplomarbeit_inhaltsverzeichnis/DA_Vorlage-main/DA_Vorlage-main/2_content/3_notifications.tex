...
% ============================================================
\chapter{Implementierung: Benachrichtigungs- und Erinnerungsmodul}
...
Skyline wurde primär für \textbf{iOS} entwickelt und ist im \textbf{Apple App Store} verfügbar \cite{appleLocalNotif}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=0.82cm, y=1cm, >=latex]
    ...
    \node[above, align=center, font=\scriptsize] at (-2.5,0.15)
      {T$-$60m\\Boarding};
...
Ohne einen Cancel-Schritt würden bei jeder Änderung (z.\,B.\ neue Abflugzeit, stornierter Flug)
die alten Reminder weiterhin zum ursprünglich geplanten Zeitpunkt ausgelöst -- der Nutzer
erhielte dann z.\,B.\ einen Boarding-Hinweis für einen bereits abgeflogenen oder
abgesagten Flug.
Der Ablauf ist daher zweistufig: Zuerst werden alle bestehenden Reminder für den Flug
gelöscht (\emph{Cancel}), anschließend werden bei Bedarf neue Reminder mit den
aktualisierten Daten neu geplant (\emph{Reschedule}).
...
vorbereitet.
Da Skyline primär für iOS entwickelt wurde und im Apple App Store verfügbar ist, bildet
...
\begin{figure}[H]
  \centering
  \roundedimg[width=0.8\textwidth]{14_expo_push_token/image}
  \caption{Gespeicherter Expo-Push-Token im Nutzerprofil (\texttt{profiles.preferences})}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[state] (fail)  at (3,-5.5)  {failed};
    ...
    \draw[arr] (sched) -- node[above,card]{cancelAutoReminders} (canc);
    \draw[arr] (pend)  -- node[lbl, pos=0.75, below, xshift=-0.5cm]{Scheduling error} (fail);
    \draw[arr] (sched) -- node[lbl, pos=0.75, below, xshift=0.5cm]{Scheduling error} (fail);
  \end{tikzpicture}
...
Die Konfiguration erfolgt über den Settings-Tab. Die App bietet getrennte Toggles
für Push Notifications allgemein sowie für Boarding-, Check-in-, Missing-Docs-,
Receipt- und Note-/Checklist-Reminder, um Überlastung und unnötige Unterbrechungen
zu vermeiden \cite{ozdemir2025echoes,visuri2019notifications}. Quiet Hours verschieben
geplante Notifications aus konfigurierten Ruhezeitfenstern heraus
(Listing~\ref{lst:quiet_hours}).

\begin{figure}[H]
  \centering
  \roundedimg[width=0.55\textwidth]{15_settings_notifications/image}
  \caption{Reminder-Einstellungen und Quiet-Hours-Konfiguration im Settings-Tab}
  \label{fig:settings_notif}
\end{figure}

\textbf{iOS:} Das lokale Notification-System erfordert ausschließlich die
Nutzer-Permission (erteilt oder verweigert beim ersten Systemdialog); eine
Channel-Konfiguration wie auf Android entfällt \cite{appleLocalNotif,expoNotificationsSDK}.

\textbf{Testfunktion:} Der Settings-Screen bietet eine ,,Send test notification''-Funktion,
die dieselben Scheduling-Helfer mit einem kurzen Offset von wenigen Sekunden verwendet
und damit iOS-Permission sowie Scheduling-Pipeline validiert. Android-Channels sind
im Code vorhanden, werden auf iOS ignoriert und wurden auf Android nicht produktiv
getestet \cite{androidNotifChannels}.
...
Zur Transparenz und Fehlersuche wurde während der Entwicklung ein Screen \emph{Pending notifications}
implementiert.
Er kombiniert lokal geplante Notifications mit serverseitig offenen Registry-Einträgen 
\cite{expoNotificationsSDK}.
Dieser Screen diente ausschließlich als Debug-Werkzeug in der Entwicklungsphase und ist in der
fertigen App nicht über die Navigation erreichbar.
Er wurde nach der Entwicklungsphase entfernt und ist in der finalen App-Version nicht mehr enthalten.
...
Die Wirkung des Benachrichtigungsmoduls wird entlang drei inhaltlicher Dimensionen
diskutiert: Zuverlässigkeit, Effizienz und Nutzerakzeptanz.
