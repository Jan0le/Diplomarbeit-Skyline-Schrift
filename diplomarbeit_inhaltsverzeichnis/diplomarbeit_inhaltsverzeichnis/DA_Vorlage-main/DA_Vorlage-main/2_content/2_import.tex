...
\chapter{Implementierung: Automatischer Import von Boardkarten \& Buchungsdaten}\label{chapter:import}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.46\textwidth]{fehlende_bilder/additionflow/image}\hfill
  \includegraphics[width=0.46\textwidth]{fehlende_bilder/additionflow/image copy}
...
\begin{lstlisting}[caption={Verarbeitung des QR-/Barcode-Scans in Skyline (vereinfacht, Import-Screen)},label={lst:handle_barcode_scanned}]
...
Aus dem Rohstring werden u.\,a.\ Name, Buchungsreferenz, Flughäfen und Flugnummer extrahiert;
das Flugdatum liegt als sogenannter \glqq Julian day of the year\grqq{} \cite{wikipediaJulianDay} vor und wird in ein
ISO-Datum konvertiert.
...
Unter dem hier verwendeten Begriff \glqq Julian day of the year\grqq{} wird die fortlaufende
Tagesnummer innerhalb eines Kalenderjahres verstanden (z.\,B.\ 1.~Jänner = Tag~1,
31.~Dezember = Tag~365 bzw.~366) \cite{wikipediaJulianDay}.
Diese Darstellung vereinfacht Datumsberechnungen und Sortierungen, da Zeitpunkte über eine
einzige Ganzzahl repräsentiert werden können.
Die Implementierung in \texttt{julianToISODate} folgt diesem vereinfachten Ansatz und
überführt die Tagesnummer in ein standardkonformes ISO-Datum; sie verwendet damit bewusst
nicht das astronomische julianische Datum, sondern die in vielen Bibliotheken übliche
Tageszählung innerhalb des Kalenderjahres.
...
\begin{lstlisting}[caption={Dokument-Import und OCR-Aufruf (Import-Screen)},label={lst:process_document}]
const processDocument = async (uri: string) => {
  ...
  try {
    ...
    // MIME-Typ aus Dateiendung/URI bestimmen (file:// kann .pdf enthalten)
    const lower = uri.toLowerCase();
    const isPdf = lower.endsWith('.pdf')
      || (lower.startsWith('file://') && lower.includes('.pdf'));
...
Relevante Bestätigungen werden vom Nutzer selbst exportiert (z.\,B.\ als PDF oder Screenshot) und anschließend hochgeladen.
...
\textbf{Flug-Import (QR/OCR/Datei):} Der Import-Flow für neue Flüge bietet drei Wege: Scan eines Boardingpasses mit der Kamera, Import eines PDFs oder Bilds aus dem Dateidialog sowie Auswahl eines Screenshots aus der Galerie. Die Datei wird per OCR ausgewertet, Flugdaten extrahiert und nach Validierung als neuer Flug gespeichert.

\textbf{Dokumente an bestehenden Flug anhängen:} Der \texttt{DocumentUploadModal} dient dem Hochladen weiterer Dokumente zu einem \emph{bereits angelegten} Flug (z.\,B.\ aus der Trip-Detailansicht oder der Dokumentenliste). \texttt{DocumentService.uploadDocument} liest die Datei, bestimmt MIME- und Dokumenttyp und legt den Binärinhalt im Supabase-Storage sowie Metadaten in \texttt{flight\_documents} ab \cite{supabaseStorage}. Die Koppelung erfolgt über \texttt{flight\_id}.

Damit ist die Buchungsbestätigung sowohl als Originaldokument verfügbar als auch -- sofern im Add-Flight-Import über OCR analysiert -- Quelle für strukturierte Flugdaten.
...
\begin{lstlisting}[caption={Synchrones Pflichtfeld-Check im Import-Screen},label={lst:import_required_fields}]
...
\begin{lstlisting}[caption={Berechnung der Flugdistanz per Haversine-Formel (Import-Screen)},label={lst:haversine}]
