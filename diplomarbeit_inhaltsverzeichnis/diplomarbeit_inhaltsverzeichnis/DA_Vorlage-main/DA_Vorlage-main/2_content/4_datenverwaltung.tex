...
% Listings-Stile (TypeScript, skyline-ts, skyline-sql) sind in setup/environments.tex definiert.

% ============================================================
\chapter{Implementierung: Zentrale \& sichere Datenverwaltung}\label{chapter:datenverwaltung}
% ============================================================
...
implementiert, sondern über einen Service-Layer gekapselt. Die zentralen Module sind
\texttt{supabase.ts} (relationale Daten) und \texttt{documentService.ts} (Storage).
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    card/.style={font=\scriptsize, fill=white, inner sep=2pt, draw=none}
  ]
    ...
    \draw[arr]  (profiles)   -- node[card, right]{1:n} (flights);
    \draw[arr]  (flights)    -- node[card, above left]{1:n} (notes);
    \draw[arr]  (flights)    -- node[card, right]{1:n} (checklists);
    \draw[arr]  (checklists) -- node[card, right]{1:n} (items);
    \draw[arr]  (flights)    -- node[card, above right]{1:n} (documents);
    \draw[sarr] (profiles)   -- node[card, below]{1:n} (notif);
    \draw[sarr] (profiles)   -- node[card, right]{1:n} (ach);
  \end{tikzpicture}%
  }
...
% --- Datenbankaufbau: projektspezifische Erklärung ---
\paragraph{Aufbau des Skyline-Datenbankschemas.}
Das Skyline-Schema gliedert sich in mehrere Bereiche:
\textbf{(1) Nutzer und Flüge:} \texttt{profiles} ist an \texttt{auth.users} gekoppelt und bildet den Einstieg für jeden Nutzer.
\texttt{user\_flights} ist die zentrale Flugentität mit FKs auf \texttt{profiles}, \texttt{airports} und optional \texttt{companies}.
\textbf{(2) Flugbezogene Inhalte:} \texttt{user\_notes}, \texttt{user\_checklists}, \texttt{user\_checklist\_items} sowie
\texttt{flight\_documents} hängen jeweils an \texttt{user\_flights} und \texttt{profiles}.
\textbf{(3) Firma und Team:} \texttt{companies} (mit \texttt{owner\_profile\_id}), \texttt{company\_members} und
\texttt{company\_invites} ermöglichen Firmenflüge und Einladungen.
\textbf{(4) E-Mail- und Kalenderintegration:} \texttt{email\_accounts}, \texttt{email\_imports} sowie
\texttt{calendar\_accounts} dienen dem automatischen Import von Buchungsdaten.
\textbf{(5) Weitere Features:} \texttt{notifications} (Push-Reminder), \texttt{achievements} und
\texttt{user\_achievements} (Gamification), \texttt{note\_templates} und \texttt{checklist\_templates} für Vorlagen.
\textbf{(6) Stammdaten:} \texttt{airports} enthält IATA/ICAO-Codes und Koordinaten für Routenberechnung.
Alle Tabellen sind über RLS an \texttt{auth.uid()} bzw.\ \texttt{profile\_id} gebunden.
...
% Screenshot Supabase Table Editor (user_flights)
\IfFileExists{img/fehlende_bilder/supabase_table_editor/image.png}{%
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{fehlende_bilder/supabase_table_editor/image}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.88\textwidth}{\centering\vspace{1.2cm}
    \textbf{Platzhalter: Screenshot einfügen}\\[0.3cm]
...
(\texttt{migration\_add\_company\_id\_to\_\allowbreak user\_flights.sql}).
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l X}
    ...
    \texttt{flight\_users}    & (Legacy-Konzept, wird in der finalen Implementierung
                                nicht verwendet; Firmenflüge werden stattdessen über
                                \texttt{user\_flights.company\_id} und
                                \texttt{company\_members} abgebildet) \\
...
\IfFileExists{img/fehlende_bilder/supabase_rls/user_flights-policies.png}{%
\begin{figure}[H]
  \centering
  \roundedimg[width=0.82\textwidth]{fehlende_bilder/supabase_rls/user_flights-policies}
  \caption{Row-Level-Security-Policies im Supabase Dashboard}
  \label{fig:supabase_rls}
\end{figure}%
}{%
\IfFileExists{img/fehlende_bilder/supabase_rls/image.png}{%
\begin{figure}[H]
  \centering
  \roundedimg[width=0.82\textwidth]{fehlende_bilder/supabase_rls/image}
  \caption{Row-Level-Security-Policies im Supabase Dashboard}
  \label{fig:supabase_rls}
\end{figure}%
}{%
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.88\textwidth}{\centering\vspace{1.2cm}
    \textbf{Platzhalter: RLS-Policies Screenshot}\\[0.3cm]
    Supabase Dashboard $\rightarrow$ Table Editor $\rightarrow$ user\_flights $\rightarrow$ RLS Policies\\
    (Policies für SELECT, INSERT, UPDATE, DELETE sichtbar)
    \vspace{1.2cm}}}
  \caption{Row-Level-Security-Policies im Supabase Dashboard}
  \label{fig:supabase_rls}
\end{figure}%
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners, thick, minimum width=3.0cm,
                minimum height=0.8cm, align=center, font=\small\ttfamily},
    flowstep/.style={draw, rounded corners, thick, minimum width=3.8cm,
                 minimum height=0.8cm, align=center, font=\small},
    arr/.style={->, thick}
  ]
    \node[flowstep] (open) at (0,0)    {Nutzer öffnet Flug};
    \node[flowstep] (par)  at (0,-1.5) {Promise.allSettled()};
    \node[box]  (n)    at (-3.5,-3){user\_notes};
    \node[box]  (c)    at (0,-3)   {user\_checklists};
    \node[box]  (d)    at (3.5,-3) {flight\_documents};
    \node[flowstep] (ui)   at (0,-4.5) {Detailansicht bereit};
    \draw[arr] (open)--(par);
...
% Storage-Bucket: Dokumentenübersicht
\IfFileExists{img/fehlende_bilder/storage_bucket/storage-bucket.png}{%
\begin{figure}[H]
  \centering
  \roundedimg[width=0.88\textwidth]{fehlende_bilder/storage_bucket/storage-bucket}
  \caption{Storage-Bucket \texttt{flight-documents}: Dokumentenübersicht im Supabase Dashboard}
  \label{fig:supabase_storage}
\end{figure}%
}{%
\IfFileExists{img/fehlende_bilder/storage_bucket/buckets.png}{%
\begin{figure}[H]
  \centering
  \roundedimg[width=0.88\textwidth]{fehlende_bilder/storage_bucket/buckets}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.88\textwidth}{\centering\vspace{1.2cm}
    \textbf{Platzhalter: Storage-Bucket (Dokumente)}\\[0.3cm]
    Supabase $\rightarrow$ Storage $\rightarrow$ Bucket \texttt{flight-documents}\\
    \vspace{1.2cm}}}
  \caption{Storage-Bucket \texttt{flight-documents}: Dokumentenübersicht}
  \label{fig:supabase_storage}
\end{figure}%
}
}
% Storage-Bucket: Policies
\IfFileExists{img/fehlende_bilder/storage_bucket/storage-bucket-policies.png}{%
\begin{figure}[H]
  \centering
  \roundedimg[width=0.88\textwidth]{fehlende_bilder/storage_bucket/storage-bucket-policies}
  \caption{Storage-Bucket \texttt{flight-documents}: Zugriffs-Policies}
  \label{fig:supabase_storage_policies}
\end{figure}%
}{%
\IfFileExists{img/fehlende_bilder/storage_bucket/image copy.png}{%
\begin{figure}[H]
  \centering
  \roundedimg[width=0.88\textwidth]{fehlende_bilder/storage_bucket/image copy}
  \caption{Storage-Bucket \texttt{flight-documents}: Zugriffs-Policies}
  \label{fig:supabase_storage_policies}
\end{figure}%
}{%
\IfFileExists{img/fehlende_bilder/storage_bucket/policies.png}{%
\begin{figure}[H]
  \centering
  \roundedimg[width=0.88\textwidth]{fehlende_bilder/storage_bucket/policies}
  \caption{Storage-Bucket \texttt{flight-documents}: Zugriffs-Policies}
  \label{fig:supabase_storage_policies}
\end{figure}%
}{}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \draw[arr] (app) -- node[above,card]{1. URL anfordern} (api);
    \draw[arr] (api) -- node[above,card]{2. Signed URL} (storage);
